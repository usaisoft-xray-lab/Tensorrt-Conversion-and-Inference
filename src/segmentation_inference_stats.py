...
def main():
    ...
    for _ in range(args.warmup):
        lb, _, _, _ = letterbox(img, size=args.size, pad_val=114)
        seg.infer_optimized(lb)
    print("Warmup complete.\n")

    # ------------------------
    # Simple timing accumulators
    # ------------------------
    simple_pre = []
    simple_gpu = []
    simple_post = []
    ...
    print(f"\nRunning {args.runs} measured inference runs...\n")
    ...
    for run_idx in range(args.runs):
        ...
        t1 = time.perf_counter()
        simple_pre.append(t1 - t0)

        # --- GPU inference ---
        boxes, labels, scores, t_inf = seg.infer_optimized(lb)
        simple_gpu.append(t_inf["gpu_infer_pure_s"])
        ...
        boxes_img = unletterbox_boxes(boxes_kept, scale, left, top,
                                      img.shape[1], img.shape[0])
        t3 = time.perf_counter()
        simple_post.append(t3 - t1)

        simple_total.append(time.perf_counter() - t_total0)

    # ------------------------
    # Print runtime summary
    # ------------------------
    print("\n" + "="*60)
    print(" SIMPLE RUNTIME SUMMARY")
    print("="*60)
    print(f"Runs: {args.runs}")
    print("-"*60)
    print(f"{'Preprocess avg:':<30} {np.mean(simple_pre)*1e3:>10.2f} ms")
    print(f"{'GPU runtime avg:':<30} {np.mean(simple_gpu)*1e3:>10.2f} ms")
    print(f"{'Postprocess avg:':<30} {np.mean(simple_post)*1e3:>10.2f} ms")
    print("-"*60)
    print(f"{'Total avg per frame:':<30} {np.mean(simple_total)*1e3:>10.2f} ms")
    print(f"{'Estimated FPS:':<30} {1.0/np.mean(simple_total):>10.2f}")
    print("="*60)

    # ------------------------
    # Generate final visualization if requested
    # ------------------------
    if args.viz == "on":
        keep = scores >= args.score
        boxes_kept = boxes[keep]
        ...
        boxes_img = unletterbox_boxes(boxes_kept, scale, left, top,
                                      img.shape[1], img.shape[0])
        if len(boxes_kept) > 0:
            masks_img, _ = seg.copy_and_resize_masks_roi_gpu(
                np.flatnonzero(keep),
                boxes_img,
                scale, left, top,
                img.shape[1], img.shape[0],
            ...
            final_vis, _, _ = colorize_blend_optimized(
                img.copy(), masks_img, boxes_img,
                labels_kept, alpha=args.alpha
            )
        else:
            final_vis = img.copy()
    else:
