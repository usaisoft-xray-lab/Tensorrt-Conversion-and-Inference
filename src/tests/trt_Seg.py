#!/usr/bin/env python3
"""
TensorRT RTMDet(-Ins) segmentation inference with clean timing, lazy mask copies,
pinned host buffers, and fast ROI visualization.

Fixes:
- argparse: --no-lazy-masks -> args.no_lazy_masks
- avoid huge host buffer for masks (device-only binding; per-run pinned scratch)
"""
...
class TRTSegmentor:
    ...
    def infer(self, lb_img_uint8, profile_copies=True):
        stream = cuda.Stream()
        ...
        if self.pinned_flag[self.name_in]:
            cuda.memcpy_htod_async(self.dev_ptr[self.name_in], self.host_buf[self.name_in], stream)
        else:
            cuda.memcpy_htod(self.dev_ptr[self.name_in], self.host_buf[self.name_in])
        if profile_copies: stream.synchronize()
        t_h1=time.perf_counter(); h2d_s=(t_h1-t_h0)

        # GPU
        start_evt, end_evt = cuda.Event(), cuda.Event()
        start_evt.record(stream)
        if self.trt10: self.context.execute_async_v3(stream.handle)
        else:          self.context.execute_async_v2(self.bindings_order, stream.handle)
        end_evt.record(stream)

        # D2H meta (async + pinned)
        t_d0=time.perf_counter()
        cuda.memcpy_dtoh_async(self.host_buf[self.name_dets],   self.dev_ptr[self.name_dets],   stream)
        cuda.memcpy_dtoh_async(self.host_buf[self.name_labels], self.dev_ptr[self.name_labels], stream)
        stream.synchronize()
        t_d1=time.perf_counter(); d2h_meta_s=(t_d1-t_d0)

        gpu_s = start_evt.time_till(end_evt)/1e3
        dets   = self.host_buf[self.name_dets].reshape(1,self.max_det,5)[0]
        labels = self.host_buf[self.name_labels].reshape(1,self.max_det   )[0]
        boxes  = dets[:,:4].astype(np.float32)
        scores = dets[:, 4].astype(np.float32)
        return boxes, labels.astype(np.int32), scores, gpu_s, h2d_s, d2h_meta_s
    ...
    def copy_masks(self, indices):
        ...
        stream = cuda.Stream()
        t0 = time.perf_counter()
        base_addr = int(self.dev_ptr[self.name_masks])  # contiguous layout (N, Hm, Wm)
        for k, idx in enumerate(indices):
            src = base_addr + int(idx) * slice_bytes
            dst = self.mask_host_buf[k].ravel()
            cuda.memcpy_dtoh_async(dst, src, stream)  # requires pinned host for real async
        stream.synchronize()
        t1 = time.perf_counter()
...
def main():
    ...
    pre_sum=gpu_sum=h2d_sum=d2h_meta_sum=d2h_masks_sum=post_sum=0.0
    ...
    for i in range(args.runs):
        ...
        # Inference
        boxes, labels, scores, gpu_s, h2d_s, d2h_meta_s = seg.infer(lb, profile_copies=True)
        gpu_sum+=gpu_s; h2d_sum+=h2d_s; d2h_meta_sum+=d2h_meta_s
    ...
    # Averages & report
    pre_avg, h2d_avg, gpu_avg = pre_sum/runs, h2d_sum/runs, gpu_sum/runs
    d2h_meta_avg, d2h_masks_avg = d2h_meta_sum/runs, d2h_masks_sum/runs
    ...
    print(f"H2D copy               : {h2d_avg   * 1e3:8.2f} ms")
    print(f"GPU inference (events) : {gpu_avg   * 1e3:8.2f} ms")
    print(f"D2H meta (dets+labels) : {d2h_meta_avg  * 1e3:8.2f} ms")
    ...
    avg_total_s = (pre_avg + h2d_avg + gpu_avg + d2h_meta_avg + d2h_masks_avg +
